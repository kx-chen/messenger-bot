...
var Options = function(messageTexts) {
    var stopEstimatesConstructor = {
        url: ('http://api.translink.ca/rttiapi/v1/stops/' + messageTexts + '?apikey=nK9aHp8kThROoJjNpcO3'),
        headers: {
            'accept': 'application/JSON'
        }
    };
    return stopEstimatesConstructor;
}

// JSON data for estimates of arrival times
var Estimates = function(stopNumber, routeNo) {
    var stopInfoConstructor = {
        url: ('http://api.translink.ca/rttiapi/v1/stops/' + stopNumber + '/estimates?apikey=nK9aHp8kThROoJjNpcO3&routeNo=' + routeNo),
        headers: {
            'accept': 'application/JSON'
        }
    };
    return stopInfoConstructor;
}
